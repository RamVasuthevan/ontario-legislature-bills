...
def transform(parliament: str, html_content: str) -> List[BillInfo]:
    ...
    for row in rows:
        ...
        url_title = ROOT_DOMAIN + bill_link_element['href'].lstrip('/') if bill_link_element else ''
        ...
        bill_info_list.append(BillInfo(parliament, bill_number, bill_title, url_title, bill_sponsors))
...
def load(bills_data: List[BillInfo]):
    with open(CSV_FILENAME, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for bill in bills_data:
            row = {'parliament': bill.parliament, 'bill_number': bill.bill_number, 'bill_title': bill.bill_title, 'url_title': bill.url_title}
...
if __name__ == "__main__":
    try:
        html_content = extract(ROOT_DOMAIN + "en/legislative-business/bills/parliament-42/session-2/")
