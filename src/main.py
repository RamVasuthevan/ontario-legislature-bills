...
def get_and_load_changed_data():
    ...
    parliamentary_session_with_possible_changes = [
        parliament
        for parliament in parliament_data_from_csv
        if parliament.parliamentary_session_name
        not in parliamentary_session_names_from_url
        or parliament.end_date == "Present"
    ]
    parliamentary_session_with_possible_changes_names = {
        parliament.parliamentary_session_name
    ...
    all_bill_data_from_parliaments_without_changes = [
        bill
        for bill in all_bill_data
        if bill.parliamentary_session_name
        not in parliamentary_session_with_possible_changes_names
    ]
    all_bill_data = (
        all_bill_data_from_parliaments_without_changes + bills_with_possible_changes
    )
    ...
    all_status_data_from_bills_without_changes = [
        status
        for status in all_status_data
        if status.parliamentary_session_name
        not in parliamentary_session_with_possible_changes_names
    ]
    all_status_data = (
        all_status_data_from_bills_without_changes + statuses_with_possible_changes
    )
